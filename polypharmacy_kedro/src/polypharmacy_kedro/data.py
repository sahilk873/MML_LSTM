...
def _load_csv_df(path: str, label: int) -> pd.DataFrame:
    df = pd.read_csv(path)
    return load_labeled_dataframe(df, label)


def load_labeled_dataframe(df: pd.DataFrame, label: int) -> pd.DataFrame:
    """Parse list-valued columns and attach the inferred label."""
    df = df.copy()
...
def _load_single_therapy_csv(path: str, label: int) -> pd.DataFrame:
    ...
    return load_single_therapy_dataframe(df, label)


def load_single_therapy_dataframe(df: pd.DataFrame, label: int) -> pd.DataFrame:
    """Normalize RENCI single-therapy dataframes to the standard schema."""
    df = df.copy()
    df = df.rename(
        columns={
...
def load_deduped_dataframe_from_frames(
    indications_df: pd.DataFrame,
    contraindications_df: pd.DataFrame,
    single_therapy_indications_df: Optional[pd.DataFrame] = None,
    single_therapy_contraindications_df: Optional[pd.DataFrame] = None,
) -> Tuple[pd.DataFrame, int]:
    """Combine multiple dataframes with labels and resolve conflicts deterministically."""
    indications = load_labeled_dataframe(indications_df, label=1)
    contraindications = load_labeled_dataframe(contraindications_df, label=0)
    frames = [indications, contraindications]

    if single_therapy_indications_df is not None:
        frames.append(load_single_therapy_dataframe(single_therapy_indications_df, label=1))
    if single_therapy_contraindications_df is not None:
        frames.append(load_single_therapy_dataframe(single_therapy_contraindications_df, label=0))
